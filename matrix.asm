...
mat_Init: MACRO
	...
	; access outside of vblank (else it won't work or will trash video)
		call	mem_SetVRAM
	ELSE
		call	mem_Set
	ENDC
...
mat_IterDeclare: MACRO
	var_LowRamByte	\1_iter_ptr	; pointer to (sub)matrix index
	var_LowRamByte	\1_iter_ptr2	; msb of matrix pointer
...
; if you pass in no arguments (aside from matrix name) the iterator will be
; set up to iterate the entire matrix
; variables are set up one-after another in ram so that they can be read
; from [de] pointer, increment de, then read next variable
mat_IterInit: MACRO
	IF _NARG == 1
	ld	bc, \1 - 1	; load address of matrix - 1
				; (we increment before sampling)
	lda	c	; load LSB of matrix address (-1)
	ld	[\1_iter_ptr], a
	lda	b	; load MSB of matrix address (-1)
	ld	[\1_iter_ptr2], a
	...
	; for a whole-matrix iteration, rowstep is 1, as that'll get us from
	; the last element of row 1 to the first element of row2, for example.
	lda	1
...
mat_IterNext: MACRO
	...
	ld	de, \1_iter_ptr	; de points to LSB of address ptr to submatrix
...
.iter_next_row	;we get here if we reached end of matrix row.
	...
	pop	de
	push	de	; retrieve (and re-store) address of \1_iter_ptr
	increment	de	; point to \1_iter_ptr2
